...
export default function HisnQuizClient({ initialData }: HisnQuizClientProps) {
    ...
    const generateQuestion = useCallback(() => {
        ...
        const randomCatIndex = Math.floor(Math.random() * initialData.length);
        const targetCategory = initialData[randomCatIndex];
        ...
        while (distractors.length < 3) {
            ...
            const distractor = initialData[idx];
    ...
    useEffect(() => {
        if (gameState === 'playing' && !currentCategory) {
            generateQuestion();
        }
    }, [gameState, currentCategory, generateQuestion]);
